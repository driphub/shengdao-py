...
class ProxyTool():
	...
	def getProxy(self, num_proxies=1, max_tries=5):
		...
		unfiltered_proxies = []
		print('正在获取代理...')
		for site in tqdm(self.free_proxy_sources):
			unfiltered_proxies += site.get()
		print('获取到'+str(len(unfiltered_proxies))+'个代理')
		print('检查代理是否可用...')
		for ip_port in tqdm(unfiltered_proxies):
			print("\r已找到%s个可用代理..."%str(len(proxies)),end="")
